...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function requestLocationPermission() {
        if (navigator.permissions) {
            ...
            startTrackingLocation(); // Fallback for browsers without Permissions API
        }
    }
    ...
    function startTrackingLocation() {
        if (navigator.geolocation) {
            ...
            // Set an interval to send the location
            setInterval(sendLocation, 300000); // 5 minutes in milliseconds
    ...
    function sendLocation() {
        ...
        // Check if current time is between 8 AM (8) and 5 PM (17)
        if (currentHour >= 8 && currentHour < 17) {
            ...
            console.log("Current time is outside of the specified range. Location not sent.");
        }
    }

    // Function to store location
    function storeLocation(latitude, longitude) {
        const id_user = localStorage.getItem('id_user') || 0; // Make sure this is defined and available
        const locationData = { latitude, longitude, id_user };

        // Store in cache for service worker
        caches.open('location-cache').then((cache) => {
            cache.put('lastLocation', new Response(JSON.stringify(locationData)));
        });

        // Send the location to service worker
        sendLocationToServer(latitude, longitude);
    }
    ...
    function sendLocationToServer(latitude, longitude) {
        if (navigator.serviceWorker.controller) {
            navigator.serviceWorker.controller.postMessage({
                ...
                latitude: latitude,
                longitude: longitude,
                id_user: localStorage.getItem('id_user') || 0, // Pass id_user here
            });
        }
    }
    ...
    function registerBackgroundSync() {
        if ('serviceWorker' in navigator && 'SyncManager' in window) {
            navigator.serviceWorker.ready.then((registration) => {
                return registration.sync.register('sendLocation');
            }).then(() => {
